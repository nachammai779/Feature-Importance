...
def get_feature(pdb, features_path, expected_n_channels):
    ...
    assert len(cov[0, 0, :]) == 441
	
    xros = np.load(features_path + '/trRos/' + pdb + '.npy')
    assert len(xros[0, 0, :]) == 526
    x_what_we_need_1 = xros[:, :, :84]
    avgtrROS_512_512_526 = np.load('/nvme2tb/nachammai/FeatureExtraction/AVGtrROS/250AVG.trROS.npy')
    avgtrROS_L_L_526 = avgtrROS_512_512_526[:l1,:l1,:84]
    x_what_we_need_1 = avgtrROS_L_L_526
    assert len(xros[0, 0, :]) == 526
    x_what_we_need_2 = avgtrROS_512_512_526[:, :, -1:]
	
    assert len(x_what_we_need_1[0, 0, :85]) == 84
    assert len(x_what_we_need_2[0, 0, -1:]) == 1
     
    x_pdnet = get_feature_55(pdb, features_path, 55)

    x = np.concatenate((cov, pre, x_pdnet, x_what_we_need_1, x_what_we_need_2), axis = 2)
    assert len(x[0, 0, :]) == expected_n_channels
